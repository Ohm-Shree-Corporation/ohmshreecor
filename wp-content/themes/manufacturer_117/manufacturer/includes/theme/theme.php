...
function pearl_load_styles($qty = 1, $param_name = 'style', $admin_label = false)
{
	$res = array();
	for ($i = 1; $i <= $qty; $i++) {
		$res[sprintf(esc_html__("Style %s", 'manufacturer'), $i)] = "style_{$i}";
	}

	$arr = array(
		'type'       => 'dropdown',
		'heading'    => esc_html__('Style', 'manufacturer'),
...
function pearl_pagination($pagination = array(), $defaults = array())
{
	...
	switch ($style) {
		case 'style_2' :
			$pagination['prev_text'] = '<i class="stmicon-arrow-prev"></i><span>' . esc_html__('PREV', 'manufacturer') . '</span>';
			$pagination['next_text'] = '<span>' . esc_html__('NEXT', 'manufacturer') . '</span><i class="stmicon-arrow-next"></i>';
			break;
		case 'style_4' :
...
function pearl_wp_link_pages()
{
	$defaults = array(
		...
		'separator'        => ' ',
		'nextpagelink'     => esc_html__('Next page', 'manufacturer'),
		'previouspagelink' => esc_html__('Previous page', 'manufacturer'),
		'pagelink'         => '%',
		'echo'             => 1
	);
...
{
	$post_formats = array(
		esc_html__('All', 'manufacturer') => 'all'
	);
...
function pearl_vc_add_shadow($prefix = 'shadow')
{
	return array(
		array(
			'type'       => 'textfield',
			'heading'    => esc_html__('Shadow X offset', 'manufacturer'),
			'param_name' => $prefix . '_x_offset',
			'group'      => esc_html__('Design Options', 'manufacturer'),
			'std'        => 0
		),
		array(
			'type'       => 'textfield',
			'heading'    => esc_html__('Shadow Y offset', 'manufacturer'),
		...
		array(
			'type'       => 'textfield',
			'heading'    => esc_html__('Shadow blur', 'manufacturer'),
		...
		array(
			'type'       => 'textfield',
			'heading'    => esc_html__('Shadow spread', 'manufacturer'),
		...
		array(
			'type'       => 'colorpicker',
			'heading'    => esc_html__('Shadow color', 'manufacturer'),
