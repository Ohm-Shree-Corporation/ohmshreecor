...
$demos = array(
    'textile'      => array(
        'label' => esc_html__('Textile', 'manufacturer'),
    ),
    'industrial'        => array(
        'label' => esc_html__('Industrial', 'manufacturer'),
    ),
    'factory'        => array(
        'label' => esc_html__('Factory', 'manufacturer'),
    ),
    'industrial2'        => array(
        'label' => esc_html__('Industrial 2', 'manufacturer'),
    ),
    'snacks'     => array(
        'label' => esc_html__('Snacks', 'manufacturer'),
    ),
    'leather'     => array(
        'label' => esc_html__('Leather', 'manufacturer'),
    ),
    'lamps'     => array(
        'label' => esc_html__('Lamps', 'manufacturer'),
    ),
    'furniture'     => array(
        'label' => esc_html__('Furniture', 'manufacturer'),
    ),
    'dashcam'     => array(
        'label' => esc_html__('Dashcam', 'manufacturer'),
    ),
    'gimbal'     => array(
        'label' => esc_html__('Gimbal', 'manufacturer'),
    ),
    'sewing'     => array(
        'label' => esc_html__('Sewing Machines', 'manufacturer'),
    ),
    'packaging'     => array(
        'label' => esc_html__('Packaging', 'manufacturer'),
    ),
    'industrial3'     => array(
        'label' => esc_html__('Industrial 3', 'manufacturer'),
    ),
    'cosmetics'     => array(
        'label' => esc_html__('Cosmetics', 'manufacturer'),
    ),
    'pharmacy'     => array(
        'label' => esc_html__('Pharmacy', 'manufacturer'),
    ),
    'paper'     => array(
        'label' => esc_html__('Paper Packaging', 'manufacturer'),
    ),
    'tubes'     => array(
        'label' => esc_html__('Pipes, Fittings & Tubes Manufacturer', 'manufacturer'),
    ),
    'gates'     => array(
        'label' => esc_html__('Automatic Gates Manufacturer', 'manufacturer'),
    ),
    'chemical'     => array(
        'label' => esc_html__('Soap, Detergent & Chemical Production Producer', 'manufacturer'),
    ),
    'bolt'     => array(
        'label' => esc_html__('Nail and Bolt Manufacturer', 'manufacturer'),
    ),
    'parts'     => array(
        'label' => esc_html__('Automotive Parts Manufacturer', 'manufacturer'),
    ),
    'ceramic'     => array(
        'label' => esc_html__('Venis & Ceramic Tile Manufacturer', 'manufacturer'),
    ),
...
<div class="wrap about-wrap stm-admin-wrap  stm-admin-demos-screen">
	...
	<?php if( !empty($auth_code) ):
		...
		<?php if (empty(!$current_demo = get_option('stm_layout'))): ?>
			<div class="stm_demo_import__notice">
				...
				<p><?php _e('If you want to import another layout, please <a href="https://wordpress.org/plugins/wordpress-reset/" target="_blank">Reset WordPress</a> database.', 'manufacturer'); ?></p>
			</div>
		<?php endif; ?>
                ...
                <label class="<?php echo esc_attr(($demo_key == $current_demo)) ? 'active' : ''; ?>">
                    <div class="inner">
						...
						<?php if ($demo_key == $current_demo): ?>
                            <div class="installed"><?php esc_html_e('Installed', 'manufacturer'); ?></div>
						<?php else: ?>
                            <div class="install"
                                 data-layout="<?php echo esc_attr($demo_key); ?>"><?php esc_html_e('Import', 'manufacturer'); ?></div>
						<?php endif; ?>
        ...
        <div class="stm_install__demo_popup">
            ...
            <div class="inner">
                ...
                <div class="privacy_policy">
                    <h4><?php esc_html_e('Privacy Policy', 'manufacturer'); ?></h4>
                    <div class="stm_install__demo_popup_close"></div>
                    <div class="stm_plugins_status">
                        <?php require_once 'privacy_policy.php'; ?>
                    </div>
                    <div class="privacy_policy_button decline" data-value="decline"><?php esc_html_e('Decline', 'manufacturer'); ?></div>
                    <div class="privacy_policy_button" data-value="accept"><?php esc_html_e('Accept', 'manufacturer'); ?></div>
                    <input type="hidden" name="skip_api" id="skip_api" value="0"/>
                </div>

                <div class="demo_install" style="display: none;">
                    <h4><?php esc_html_e('Demo Installation', 'manufacturer'); ?></h4>
                    <div class="stm_install__demo_popup_close"></div>
                    <div class="stm_plugins_status">
                        <?php foreach ($plugins as $plugin):
                            $active = (pearl_check_plugin_active($plugin['slug'])) ? 'installed' : 'not-installed';
                            $active_text = ($active == 'installed') ? esc_html__('Installed & Activated', 'manufacturer') : esc_html__('Not installed', 'manufacturer');
                            ?>
                        ...
                        <div class="stm_content_status">
                            ...
                            <div class="title"><?php esc_html_e('Demo content', 'manufacturer'); ?></div>
                            <div class="status"><span></span>
                                <div class="loading-dots"></div>
                            </div>
                        </div>
                    </div>
                    <div class="stm_install__demo_start"><?php esc_html_e('Setup Layout', 'manufacturer'); ?></div>
                </div>
	...
	<?php else: ?>
        <div class="stm-admin-message">
			<?php printf(wp_kses_post(__('Please enter your Activation Token before running the Manufacturer.', 'manufacturer'))); ?>
        </div>
	<?php endif; ?>
...
<script type="text/javascript">
    (function ($) {
        ...
        function performAjax(plugin_slug) {
            installation = true;
            var installing = "<?php esc_html_e('Installing', 'manufacturer'); ?>";
            var installed = "<?php esc_html_e('Installed & Activated', 'manufacturer'); ?>";
            ...
            $.ajax({
                ...
                data: {
                    ...
                    'security': window.wp_data.pearl_install_plugin,
                },
                beforeSend: function () {
                    $current_plugin
                        .addClass('installing')
        ...
        function install_demo() {
            installation = true;
            var importing_demo_text = "<?php esc_html_e('Importing Demo', 'manufacturer'); ?>";
            var imported_demo_text = "<?php esc_html_e('Imported', 'manufacturer'); ?>";
